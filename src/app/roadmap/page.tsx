...
const milestones: Milestone[] = [
  ...
  },
  {
    ...
    tagline:
      "7 new chart types, embed mode, and time intelligence. 18 charts total covering every analytics use case from scatter plots to geographic maps.",
    status: "next",
    icon: Eye,
    highlights: [
      "ScatterPlot — correlation analysis with reference lines and node sizing",
      "Treemap — hierarchical breakdowns with multiple tiling algorithms",
      "Calendar — GitHub-style contribution heatmap with auto date range",
      "Radar — multi-dimensional comparison across metrics",
      "Sankey — flow visualization with gradient links and node toggling",
      "Choropleth — geographic data on custom GeoJSON maps",
      "Bump — ranking changes over time with start/end labels",
      "Embed mode — render complete dashboards inside any React app",
      "GranularitySelector — day/week/month rollup toggle",
    ],
    newComponents: [
      "ScatterPlot",
      "Treemap",
  ...
  },
  {
    ...
    tagline:
      "Dashboards that write back. Toggle switches, inline edits, form inputs — Retool-style interactivity where user actions update your database. Not just read, but read-write.",
    status: "planned",
    icon: Workflow,
    highlights: [
      "Interactive inputs — toggles, dropdowns, text fields that bind to dashboard state",
      "Action buttons — trigger API calls, database writes, or workflows from the dashboard",
      "Inline editing — click a table cell or KPI value to update it directly",
      "Form components — structured data entry that integrates with the filter/state system",
      "Webhook actions — fire HTTP requests on user interactions for backend integration",
    ],
    newComponents: ["ActionButton", "InlineEdit", "FormField", "WebhookAction"],
  },
  {
    version: "0.9",
    title: "Rock Solid",
